...
export class DatabaseSeederService implements OnModuleInit {
  ...
  private async applyBaseSeed(queryRunner: QueryRunner): Promise<void> {
    const sql = fs.readFileSync(this.resolveAssetPath('seed.sql'), 'utf8');
    const statements = this.extractSqlStatements(sql).map((statement) =>
      this.makeStatementIdempotent(statement),
    );
  ...
  private loadSeedCounts(): LoadSeedCounts {
    const yamlPath = this.resolveAssetPath('load-seed.yaml');
  ...
  private resolveAssetPath(fileName: 'seed.sql' | 'load-seed.yaml'): string {
    const candidates = [
      path.join(__dirname, fileName),
      path.join(process.cwd(), 'libs/shared/database/src', fileName),
    ];

    const resolved = candidates.find((candidate) => fs.existsSync(candidate));

    if (!resolved) {
      throw new Error(`Unable to resolve database seeder asset "${fileName}".`);
    }
  ...
  private async seedProductos(
    ...
    await this.batchInsert(
      ...
      insertedIds.map((id, index) => {
        ...
        return [
          id,
          `LOAD-${String(sequence).padStart(5, '0')}`,
          `${7000000000000 + sequence}`,
          `Producto Load ${sequence}`,
          this.marcas[index % this.marcas.length],
          this.categorias[index % this.categorias.length],
          `Presentacion ${((index % 3) + 1).toString()}`,
  ...
  private async seedPedidos(
    ...
    await this.batchInsert(
      ...
      insertedIds.map((id, index) => {
        ...
        return [
          id,
          `PED-LOAD-${String(sequence).padStart(5, '0')}`,
          this.tiendaIds[index % this.tiendaIds.length],
    ...
    await this.batchInsert(
      ...
      insertedIds.map((id, index) => {
        ...
        return [
          id,
          this.tiendaIds[index % this.tiendaIds.length],
          `EXT-${8000000000000 + sequence}`,
          `Producto Externo Load ${sequence}`,
          this.categorias[index % this.categorias.length],
  ...
  private async getCatalogoProductoPairs(
    ...
    return new Set(
      rows.map((row) =>
        this.catalogoProductoPairKey(row.catalogosId, row.productosId),
      ),
    );
  }
  ...
  private buildMissingCatalogoProductoPairs(
    ...
    for (const catalogoId of catalogoIds) {
      for (const productoId of productoIds) {
        const pairKey = this.catalogoProductoPairKey(catalogoId, productoId);
  ...
  private catalogoProductoPairKey(
    catalogoId: string,
    productoId: string,
  ): string {
    return `${catalogoId}:${productoId}`;
  }
